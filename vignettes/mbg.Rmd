...
communes <- sf::st_read(system.file('extdata/Benin_communes.gpkg', package = 'mbg'))

# Convert point data to sf
outcome_sf <- sf::st_as_sf(outcomes, coords = c('x', 'y'), crs = sf::st_crs(communes))
...
plot(sf::st_geometry(communes), col = NA, border = 'black', add = TRUE)
```
...
plot(
  commune_summary_sf['mean'],
  main = 'MBG mean estimates by commune',
  pal = terra::map.pal('viridis', 6),
  breaks = seq(0.15, .45, by = .05)
)
```
...
commune_summary_sf$ui <- commune_summary_sf$upper - commune_summary_sf$lower
plot(
  commune_summary_sf['ui'],
  main = 'MBG 95% uncertainty interval width by commune'
)
```
