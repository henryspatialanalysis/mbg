...
Load the `mbg` package, as well as the `data.table`, `sf`, and `terra` packages for data processing.
...
For this tutorial, we will estimate stunting rates among children under 5 years of age across Tanzania. Load point observations of this outcome from the package data and print the first few rows:
...
The outcome data has 603 rows, each representing one surveyed location, and five columns:
...
Let's map this survey data over a map of Tanzania.

```{r plot-outcome-data}
# Administrative boundaries
districts <- sf::st_read(system.file('extdata/Tanzania_districts.gpkg', package = 'mbg'))

# Convert point data to sf
outcome_sf <- sf::st_as_sf(outcomes, coords = c('x', 'y'), crs = sf::st_crs(districts))
outcome_sf$stunting_rate <- outcome_sf$indicator / outcome_sf$samplesize

plot(outcome_sf['stunting_rate'], pch = 16, main = 'Stunting point data', reset = FALSE)
plot(sf::st_geometry(districts), col = NA, border = 'black', add = TRUE)
```

We can see that there seem to be spatial patterns in the surveyed stunting data---stunting generally seems to be lowest in the coastal east of the country and on the islands, and higher in the west. To better understand the underlying trends in stunting risk across the country, we would ideally like to:
...
2. `aggregation_table` (`data.table::data.table`, optional): Used to aggregate model estimates from grid cells to district polygons, preserving uncertainty
...
aggregation_table <- mbg::build_aggregation_table(
  polygons = districts,
  id_raster = id_raster,
  polygon_id_field = 'district_code'
)
...
model_runner <- MbgModelRunner$new(
  ...
  aggregation_levels = list(
    district = c('district_code', 'district'),
...
Compare these gridded estimates to the underlying point data. The results seem to meet our goals for estimating stunting risk across Tanzania:

1. Gridded estimates are available for every region in Tanzania, not just at survey points
2. The mean estimates for stunting risk show a similar spatial pattern to the underlying point data---lower on the coast and islands, higher in the west and northwest
...
We can merge the district estimate summaries back onto the `districts` sf object to plot the results.

```{r plot-district-mean-predictions}
# Get predictions by district
aggregated_predictions <- model_runner$aggregated_predictions
district_summary <- aggregated_predictions$district$summary
district_summary_sf <- merge(
  x = districts,
  y = district_summary,
  by = c('district_code', 'district')
)
# Plot aggregated estimates by district
plot(
  district_summary_sf['mean'],
  main = 'MBG mean estimates by district',
  pal = terra::map.pal('viridis', 9),
  nbreaks = 10
)
```

We can also plot the width of the 95% uncertainty interval by district.

```{r plot-district-ui-widths}
# Plot aggregated uncertainty interval widths by district
district_summary_sf$ui <- district_summary_sf$upper - district_summary_sf$lower
plot(
  district_summary_sf['ui'],
  main = 'MBG 95% uncertainty interval width by district'
)
```
