...
predictive distribution p\_{posterior}(\theta) against some new data
\tilde{y} drawn from the true underlying distribution. If we had this
...
across *N* observations to get the overall *predictive density (PD)*:

PD = p(\tilde{y}\|\theta) = \prod\_{i=1}^{N} p(\tilde{y}\_i \| \theta)
...
LPD = \sum\_{i=1}^N \log{ p(\tilde{y}\_i \| \theta) }
...
\widehat{LPD} = \sum\_{i=1}^{N} \log \left( \frac{1}{S} \sum\_{s=1}^{S}
p(\tilde{y}\_i \| \theta_s) \right)

In the notation above, we are repeatedly evaluating each data point i
(for i = 1 to *N*) against each predictive posterior draw *s* (for *s* =
1 to *S*).
...
In this context, y_i is the observed *rate* of the outcome (calculated
as the number of positives divided by the sample size) at data point i,
while \hat{y}\_i is the model’s *mean prediction* at the pixel that
overlaps with data point i. RMSE can be calculated “in-sample” against
data that was used to fit the model, or “out-of-sample” against data
that was held back from the model.
...
- The uncertainty of model estimates is not taken into account: two
  ...
  intervals would end up with the same \hat{y}\_i values and therefore
  identical RMSE.
...
In **leave one out cross validation**, we evaluate each data point y_i
against a model that was trained on the entire dataset *except* for y_i;
...
unobserved dataset drawn from the same distribution.
...
[introductory
vignette](https://henryspatialanalysis.github.io/mbg/articles/mbg.md),
to a stacked ensemble model with department-level fixed effects, as
described in the [spatial ML models
article](https://henryspatialanalysis.github.io/mbg/articles/model-comparison.md).
